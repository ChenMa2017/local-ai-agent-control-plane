...
27. render_report.py will re-run watchdog_doc_search.py locally and reject current_conclusion_update if the verified retrieval decision is not safe_to_answer or if the exact retrieval query is not registered in project_index/golden_queries.json with expected_decision=safe_to_answer. Do not claim a durable conclusion when retrieval still warns about stale, auxiliary-only, conflicting, missing, or unregistered evidence.
...
38. When a route or task is expected to support a durable current_conclusion_update, keep TASK_BOX/ROUTE_CANONICAL aligned with the same current_conclusion_topic_id and current_conclusion_query that you later use in current_conclusion_evidence_search, and register that exact query in project_index/golden_queries.json with expected_decision=safe_to_answer.
