...
async function main() {
  ...
  assert.strictEqual(nextTaskDraft.provenance.source, "model_authored");
  assert.strictEqual(nextTaskDraft.provenance.repair_origin, "model_payload.successor_task_draft");
  assert.strictEqual(nextTaskDraft.provenance.parent_route_epoch, "route-002");
  assert.strictEqual(nextTaskDraft.provenance.derived_from_report.path, "agent/reports/latest.md");
  ...
  assert.strictEqual(taskProfileDraft.provenance.source, "model_authored");
  ...
  assert.strictEqual(queueDraft.provenance.source, "model_authored");
  ...
  assert.strictEqual(routeCanonical.successor_provenance.successor_task_draft.source, "model_authored");
  const mirroredState = JSON.parse(fs.readFileSync(path.join(projectRoot, "agent", "STATE.json"), "utf8"));
  assert.strictEqual(mirroredState.successor_provenance.successor_task_draft.source, "model_authored");
  ...
  const gateStatus = JSON.parse(fs.readFileSync(path.join(projectRoot, "agent", "status", "GATE_STATUS.json"), "utf8"));
  assert.strictEqual(gateStatus.hard_blocked, false);
  assert.strictEqual(gateStatus.experiment_decision_gate.status, "not_required");
  assert.strictEqual(gateStatus.current_conclusion_gate.status, "satisfied");
  assert.strictEqual(gateStatus.current_conclusion_gate.golden_query_status, "matched_safe_to_answer");
  assert.strictEqual(gateStatus.current_conclusion_gate.evidence_decision, "safe_to_answer");
  const gateStatusText = fs.readFileSync(path.join(projectRoot, "agent", "status", "GATE_STATUS.md"), "utf8");
  assert.match(gateStatusText, /Hard blocked: false/);
  assert.match(gateStatusText, /Current conclusion gate status: satisfied/);
  assert.match(gateStatusText, /Current conclusion golden query: matched_safe_to_answer/);
  ...
  assert.strictEqual(fallbackDraft.provenance.source, "fallback_synthesized");
  assert.strictEqual(fallbackDraft.provenance.repair_origin, "successor_contract_required_missing_task");
  ...
  assert.strictEqual(fallbackRouteCanonical.successor_provenance.successor_task_draft.source, "fallback_synthesized");
  ...
  assert.strictEqual(fallbackProfile.provenance.source, "fallback_synthesized");
  ...
  assert.strictEqual(fallbackTaskBox.successor_provenance.successor_task_draft.source, "fallback_synthesized");
  ...
  assert.match(fallbackCurrentState, /Successor task source: fallback_synthesized/);
  ...
  assert.match(fallbackNextAction, /Successor task origin: successor_contract_required_missing_task/);
  ...
  assert.strictEqual(fallbackStateMirror.successor_provenance.successor_task_draft.source, "fallback_synthesized");
  const fallbackRunState = JSON.parse(fs.readFileSync(path.join(projectRoot, "agent", "RUN_STATE.json"), "utf8"));
  assert.strictEqual(fallbackRunState.successor_provenance.successor_task_draft.source, "fallback_synthesized");
  route = runRoute(projectRoot);
  assert.strictEqual(route.primary_skill, "watchdog-orchestrator");
  ...
  const gateBlockedStatus = JSON.parse(fs.readFileSync(path.join(projectRoot, "agent", "status", "GATE_STATUS.json"), "utf8"));
  assert.strictEqual(gateBlockedStatus.hard_blocked, true);
  assert.strictEqual(gateBlockedStatus.experiment_decision_gate.status, "blocked");
  assert.ok(gateBlockedStatus.blocking_reasons.some((item) => /experiment decision gate is explicitly blocked/i.test(item)));
  const gateBlockedStatusText = fs.readFileSync(path.join(projectRoot, "agent", "status", "GATE_STATUS.md"), "utf8");
  assert.match(gateBlockedStatusText, /Hard blocked: true/);
  assert.match(gateBlockedStatusText, /Experiment gate status: blocked/);
  route = runRoute(projectRoot);
  assert.strictEqual(route.primary_skill, "watchdog-orchestrator");
  ...
  assert.strictEqual(canonicalCpuDraft.provenance.source, "route_repair");
  assert.strictEqual(canonicalCpuDraft.provenance.repair_origin, "exact_contract_rehydration");
  ...
  assert.strictEqual(canonicalCpuTaskBox.successor_provenance.successor_task_draft.source, "route_repair");
  ...
  assert.strictEqual(canonicalCpuState.successor_provenance.successor_task_draft.parent_route_epoch, "route-canonical-only-001");
  const canonicalCpuCurrentState = fs.readFileSync(path.join(projectRoot, "agent", "CURRENT_STATE.md"), "utf8");
  assert.match(canonicalCpuCurrentState, /Successor task source: route_repair/);
