...
`command_prefix` controls the slash command namespace. Discord slash commands are not isolated per channel, so multiple bots in the same guild should use different prefixes, for example:
...
That produces commands like `/server_agent_run`, `/server_agent_status`, and `/server_agent_task`. Channel allowlists still protect execution, but they do not hide commands from Discord's slash command picker.
...
For `done` and `policy_violation`, the notification uses the default safe result from `/codex/result` and truncates it to `max_result_chars`. `policy_violation` is how `codex-bridge` reports a workspace-write task that touched a protected path policy. For other terminal states, it sends a short safe status summary. Raw results and raw logs are never posted to Discord.
